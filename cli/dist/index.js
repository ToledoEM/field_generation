...
program
  ...
  .option('--seed <number>', 'Seed (placeholder, not yet deterministic)', v=>parseInt(v))
...
function run(opts) {
  ...
  // PNG rasterization (pure JS) with gradient coloring
  ...
  function lerpColor(a,b,t){
    ...
    const brc=parseInt(b.slice(1,3),16), bgc=parseInt(b.slice(3,5),16), bbc=parseInt(b.slice(5,7),16);
    const rr=Math.round(ar + (brc-ar)*t), rg=Math.round(ag + (bgc-ag)*t), rb=Math.round(ab + (bbc-ab)*t);
    return [rr,rg,rb];
  }
