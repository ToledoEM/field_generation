...
export function generateField(opts) {
  ...
  // Apply seed (non-deterministic placeholder; real perlin should use seeded lib)
  if (seed !== null) {
    setSeed(seed);
  }
  ...
  // Build sources if method needs them
...
export function toSVG(data) {
  ...
  // Build gradients per path using palette sequence interpolation
  function gradientStops(count){
    const stops=[];
    for (let i=0;i<count;i++) {
      const t = count===1?0: i/(count-1);
      // Interpolate across palette array
...
export function resolvePalette(key) {
  ...
  const idx = parseInt(key.replace('p','')); // expect p0..p9
